...
const inputJson = ref('[{"name":"John","age":30},{"name":"Jane","age":25}]')
const outputCsv = ref('')
const error = ref('')
...
const convert = () => {
  ...
  try {
    ...
    outputCsv.value = Papa.unparse(data)
  } catch (e: any) {
  ...
  <div class="p-4 h-[calc(100vh-4rem)] flex flex-col">
    ...
    <div class="grid md:grid-cols-2 gap-4 flex-1 min-h-0">
      <div class="flex flex-col gap-2 min-h-0">
        ...
        <Textarea v-model="inputJson" placeholder="Paste JSON array..." class="flex-1 font-mono resize-none" />
      </div>
      <div class="flex flex-col gap-2 min-h-0">
        <div class="flex justify-between items-center">
          <Label>Output CSV</Label>
          <Button variant="ghost" size="sm" @click="copyToClipboard" :disabled="!outputCsv">
            <Copy class="h-4 w-4 mr-2" />
            Copy
          </Button>
        </div>
        <Textarea v-model="outputCsv" readonly placeholder="Result..." class="flex-1 font-mono resize-none bg-muted" />
      </div>
    </div>
